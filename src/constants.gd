...
class EnemyConstants:
	...
	const MAX_SIZE_LEVEL: int = 10  ## 最大体型等级
	...
	static var SIZE_MAP = {
		...
		SIZE_LEVEL_10: Vector2(128, 128) ## 128×128 - 大型敌人，容易击中但血量很高
	}
	...
	const SPAWN_INTERVAL_MIN: float = 0.15  ## 【核心参数】最小刷新间隔（秒），后期最快速度。越小=越难
	const SPAWN_INTERVAL_MAX: float = 0.8   ## 【核心参数】最大刷新间隔（秒），游戏开始时的初始速度。越小=前期越难
	const SPAWN_INTERVAL_MID: float = 0.35  ## 中期目标刷新间隔，超过此值后减缓难度增长
	const SPAWN_LERP_SPEED: float = 0.15    ## 刷新间隔变化平滑度（0-1），越大=难度变化越快
	
	# ============================================
	# 体型解锁配置（控制敌人体型成长）
	# ============================================
	## 这些参数控制体型等级随时间的解锁速度
	const TIME_SCALE_FACTOR: float = 0.7    ## 体型权重时间加成系数，越大=大体型出现越早
	const MAX_SIZE_UNLOCK_TIME: float = 120.0  ## 【核心参数】完全解锁所有体型所需时间（秒）。越小=体型成长越快
	const SIZE_UNLOCK_POWER: float = 0.7    ## 体型解锁曲线幂函数（<1=前期解锁快，后期慢；>1=相反）
	
	# ============================================
	# 难度曲线参数（控制整体难度节奏）
	# ============================================
	## 这些参数控制难度随时间的变化曲线
	const TIME_PROGRESS_POWER: float = 0.7  ## 时间进度曲线（<1=前期难度涨得快；>1=后期涨得快）
	const SIZE_WEIGHT_POWER: float = 1.1    ## 体型权重曲线幂函数，影响大体型的出现概率
	const LATE_GAME_START_TIME: float = 45.0  ## 后期游戏开始时间（秒），超过后启用后期加成
	const LATE_GAME_BONUS_FACTOR: float = 0.25  ## 后期游戏加成系数，越大=后期大体型越多
	
	# ============================================
	# 前期压力配置（游戏开始时的额外难度）
	# ============================================
	## 用于给游戏前期增加压力，让玩家快速建立防线
	const EARLY_GAME_DURATION: float = 15.0  ## 前期持续时间（秒），在此期间有额外压力
	const EARLY_GAME_SPAWN_BONUS: float = 0.35  ## 前期额外刷怪速度加成（0.35=刷怪快35%）
	...
	## 根据玩家击杀表现动态调整难度
	## 表现越好，难度越高，防止玩家挂机
	const PERFORMANCE_HIGH_THRESHOLD: int = 300  ## 高表现分数阈值（击杀分超过此值）
	const PERFORMANCE_HIGH_FACTOR: float = 1.4   ## 高表现难度系数（1.4=刷怪快40%）
	const PERFORMANCE_MED_THRESHOLD: int = 120   ## 中等表现分数阈值
	const PERFORMANCE_MED_FACTOR: float = 1.2    ## 中等表现难度系数（1.2=刷怪快20%）
	
	# ============================================
	# 波次系统配置
	# ============================================
	## 波次系统定期增加难度，给玩家阶段性压力
	const WAVE_INTERVAL: float = 20.0       ## 【核心参数】波次间隔（秒）。越小=波次越频繁=越难
	const WAVE_BONUS_FACTOR: float = 1.8    ## 波次强化因子，影响波次期间敌人强度
	const WAVE_ENEMY_COUNT: int = 8         ## 【核心参数】波次额外敌人数量。越多=波次压力越大
	const WAVE_DIFFICULTY_INCREASE: int = 1 ## 每波增加的基础难度等级
	const WAVE_EXTRA_DIFFICULTY_INTERVAL: int = 2  ## 每N波额外增加一次难度
	const WAVE_SPAWN_INTERVAL_MULTIPLIER: float = 0.8  ## 波次期间刷新间隔乘数（0.8=快20%）
	const WAVE_BONUS_PER_WAVE: float = 0.15 ## 每波体型解锁进度加成（0.15=快15%）
	const WAVE_SIZE_BONUS_POWER: float = 1.3  ## 波次体型权重幂函数
	const WAVE_SIZE_BONUS_FACTOR: float = 0.7 ## 波次体型权重系数
	...
	static var ENEMY_SCORE_MAP = {
		...
		SIZE_LEVEL_10: 150   ## 10级：150分 - 难击杀，分数高
	}
